...
Функция ОтправитьPUTОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		ПолученныйДиапазон = ПередачаДанныхСлужебный.ПолученныйДиапазон(Запрос);
		...
		Иначе
			...
			Если ОписаниеДанных.УдалитьФайлДанных Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(НСтр("ru = 'ПередачаДанных'", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,,, ПередачаДанныхКлиентСервер.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
